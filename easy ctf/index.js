...
//DON'T USE THIS TO FIND FLAW!!!

//tous les commentaires sont là pr montrer ce que j'ai modifié pour générer une faille
...
io.on('connection', function(socket) {
    socket.on('getSession', () => {
        ...
        if(session) {                                                               //on modifie pseudo par email pr la faille
            socket.emit("getSessionResult", {session: {session: session, email: userDB.find({userID: cookies.userID}).value().email}, status: 200});
        }
        else {
    ...
    //pr la faille sinon inutile
    socket.on("getUserInfos", data => {
        var user = userDB.find({email: data.email}).value();
        if(user) {
            socket.emit("getUserInfosResult", {pseudo: user.pseudo, userID: user.userID, email: user.email, createdAt: user.createAt, status: 200});
        }
        else {
            socket.emit("getUserInfosResult", {status: 400});
        }
    });

    //pr la faille sinon inutile
    socket.on("actSession", data => {
        var session = sessions.find(a => a.userID == data.userID);
        if((data.key == "userID" || data.key == "localIP" || data.key == "token" || data.key == "expireDate") && session) session[data.key] = data.value;
    });
